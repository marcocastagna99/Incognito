...
def generate_data(num_tuples):
    ...
    for i in range(num_tuples):
        ...
        # Genera un numero casuale di patologie per ogni paziente (tra 1 e 3)
        num_diseases = random.choices([1, 2, 3], weights=[0.8, 0.1, 0.1])[0]
        # Scegli le patologie in modo casuale
        diseases = random.sample(['Hypertension', 'Arthrosis/Arthritis', 'Asthma', 'Pneumonia', 'Diabetes'], num_diseases)
        while len(diseases) < 3:
...
if __name__ == "__main__":
    ...
    with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        csvwriter.writerow(['PatientID', 'DateandTime', 'Gender', 'Age', 'ZIPCode', 'Height', 'Weight', 'Disease1', 'Disease2', 'Disease3'])
